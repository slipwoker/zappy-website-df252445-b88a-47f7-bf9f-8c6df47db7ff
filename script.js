  ...
  if (contactForm) {
    contactForm.addEventListener('submit', function(e) {
      ...
      inputs.forEach(function(input) {
        ...
        if (input.type === 'tel' && input.value.trim() && input.hasAttribute('pattern')) {
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "הכל לבית משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            const response = await fetch('http://localhost:5001/api/email/contact-form', {
                ...
                body: JSON.stringify({
                    websiteId: 'df252445-b88a-47f7-bf9f-8c6df47db7ff',
                    name: data.name || '',
                    email: data.email || '',
